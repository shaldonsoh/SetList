...
export default function EditGear({ params }: PageProps) {
  const router = useRouter();
  const { listings, getUserListings, updateListing } = useListings();
  ...
  useEffect(() => {
    ...
    if (listing) {
      ...
      alert('Listing not found or you do not have permission to edit this listing');
      router.push('/listings');
      return;
    }
    
    setIsLoading(false);
  }, [isAuthenticated, params.id, router, getUserListings]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const updatedListing = {
      ...
      image: images[0] || 'https://images.unsplash.com/photo-1589872307379-0ffdf9829123',
    };

    // Update the listing
    updateListing(params.id, updatedListing);
          ...
          <form onSubmit={handleSubmit} className="space-y-6 bg-white p-6 rounded-lg shadow">
            ...
            <div className="flex gap-4">
              <button
                ...
                className="flex-1 py-3 px-4 border border-gray-300 rounded-lg shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-amber-500"
              >
                Cancel
              </button>
              <button
                type="submit"
                className="flex-1 py-3 px-4 border border-transparent rounded-lg shadow-sm text-sm font-medium text-white bg-black hover:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-amber-500"
              >
                Save Changes
              </button>
            </div>
